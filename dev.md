...
| 模块       | 技术选型                                      | 理由                                                                 |
| ---------- | --------------------------------------------- | -------------------------------------------------------------------- |
| SSH 库     | **Scrapli**（主）+ Netmiko（兼容）            | Scrapli 原生异步，高并发性能强；Netmiko 作为备选方案处理特殊厂商     |
| 并发框架   | **Nornir 3.x**                                | 专为网络自动化设计，内置并发、失败重试、结果聚合，简化大规模设备管理 |
| 数据解析   | **TextFSM + ntc-templates**                   | 将非结构化命令输出（show 命令）转为结构化数据，支持主流厂商          |
| 配置模板   | **Jinja2**                                    | 生成标准配置块，用户仅输入变量（VLAN ID、IP 等），避免直接输入命令   |
| 任务队列   | **Celery + Redis**                            | 成熟稳定，故障恢复机制完善，社区活跃                                 |
| 网络扫描   | **Nmap**（详细扫描）+ Masscan（快速存活探测） | Nmap 功能全面支持 OS 识别，Masscan 用于大网段快速发现                |
| 配置存储   | **PostgreSQL JSONB + MinIO**                  | 小配置存 JSONB 支持索引查询，大配置压缩后存对象存储                  |
| 拓扑可视化 | **vis.js / ECharts**                          | 支持交互式拓扑图，可点击节点查看详情                                 |
...
- 设备密码使用 AES-256 对称加密存储到数据库专用字段
- 加密密钥存储在服务器配置文件（权限 600）或环境变量
- 建立独立的凭据审计日志（记录密码修改、使用时间、操作人）
- 支持设备分组使用不同凭据（核心层、接入层）
- 为 NCM 系统创建专用只读账号（权限最小化原则）

**厂商兼容层**

- 建立设备类型自动检测机制（通过 SSH Banner 识别）
- 针对不同厂商封装统一的命令适配器
- 支持自定义厂商命令模板（应对小众设备）
...
- [ ] 集成 **TextFSM + ntc-templates**（验证常用命令解析）
- [ ] 建立 **Jinja2 模板库**基础结构（模板存储表、版本管理）
- [ ] 建立数据库表结构（Device、Backup、Task、Discovery、Template 等）
- [ ] 实现凭据加密存储与解密模块
- [ ] 搭建 Scrapli 和 Netmiko 基础连接测试
...
- [ ] 实现 Scrapli 多厂商设备连接适配层
- [ ] 使用 **Nornir** 封装批量备份任务（支持并发、失败重试）
- [ ] 开发配置备份任务（定时 + 手动）
- [ ] 实现配置版本管理与历史查看
- [ ] 使用 **TextFSM** 解析 ARP 表和 MAC 地址表
- [ ] 开发 ARP 表和 MAC 地址表采集模块（基于 Nornir 批量执行）
- [ ] 实现 IP/MAC 精准定位功能（前端查询页面）
- [ ] 完成备份任务失败重试机制

**里程碑**：运维人员可通过系统快速定位 PC 位置，减少现场排查时间
...
- [ ] 实现 OUI 数据库厂商识别
- [ ] 开发扫描结果与 CMDB 比对逻辑
- [ ] 使用 **TextFSM** 采集并解析 LLDP/CDP 邻居信息
- [ ] 使用 **Nornir** 批量采集拓扑数据（LLDP、ARP、MAC 表）
- [ ] 使用 vis.js 绘制交互式拓扑图
- [ ] 实现拓扑图节点点击查看详情

**里程碑**：自动发现影子资产，可视化展示网络拓扑结构
...
- [ ] 设计 **Jinja2 模板库**（常用配置场景）
- [ ] 开发模板管理界面（创建、编辑、版本控制）
- [ ] 实现模板参数校验与预览功能
- [ ] 开发命令白名单校验模块
- [ ] 实现三级审批流程（集成 RBAC）
- [ ] 开发 Dry-Run 试运行功能（基于 Jinja2 渲染预览）
- [ ] 使用 **Nornir** 实现灰度发布机制（测试 → 分批执行）
- [ ] 开发自动回滚功能
- [ ] 完善操作审计日志

**里程碑**：运维人员可安全批量修改设备配置，减少重复劳动
...
- 基础信息：IP、主机名、厂商、型号、位置
- 凭据信息：加密后的用户名密码（独立字段）
- 分组信息：设备分组、重要性等级（核心/汇聚/接入）
...
- Template.vendor + Template.device_type 复合索引
...
- 建立设备类型自动检测机制
- 使用 ntc-templates 覆盖主流厂商（700+ 模板）
...
**风险**：设备输出格式变化导致 TextFSM 解析失败

**应对**：

- 解析失败时记录原始输出到日志
- 提供降级方案（保存原始文本供人工分析）
- 支持自定义 TextFSM 模板快速修复
...
本规划书基于现有 Vue 3 + FastAPI 架构，通过引入 **Celery + Nornir + TextFSM + Jinja2** 技术栈，实现了安全、高效的网络配置管理系统。核心技术价值：

1. **Nornir**：专为网络自动化设计，简化批量设备管理，内置并发控制和失败容错
2. **TextFSM + ntc-templates**：700+ 现成模板，将非结构化命令输出转为结构化数据，无需手写正则
3. **Jinja2**：模板化配置生成，禁止直接输入命令，确保配置标准化和安全性
4. **Celery**：异步任务调度，解决大规模设备管理性能瓶颈
5. **Scrapli**：原生异步 SSH 库，高并发性能强

核心亮点：

- **多重安全机制**：Jinja2 模板 + 白名单 + 审批流 + 灰度发布
- **智能数据解析**：TextFSM 自动解析 ARP/MAC/LLDP 表为结构化数据
- **高效并发执行**：Nornir + Scrapli 实现 1000+ 设备快速备份
- **精准定位**：快速解决"PC 在哪个端口"的运维痛点
- **拓扑可视化**：直观展示网络结构
- **生命周期联动**：自动验证资产状态，发现影子资产

按照 Phase 0 → Phase 5 的开发路线逐步推进，既能快速见效（Phase 1 的精准定位），又能循序渐进完善功能，降低开发风险。
