...
基于 **Vue 3 + FastAPI** 的网络配置管理系统，内置完整的 RBAC 权限控制与后台管理能力，并提供网络自动化相关能力（配置备份、采集、巡检等）。
...
- **网络自动化**：配置备份、资产发现/采集、异步任务、命令审计、拓扑发现（以实际模块为准）
...
建议：本地开发优先使用 uv 管理 Python 依赖、pnpm 管理前端依赖。
...
# Windows PowerShell 可用：Copy-Item .env.example .env.development
...
### Alembic 迁移报 Multiple head revisions
...
- 如果输出包含多个 head，需要创建 merge revision 合并迁移分叉；本仓库如已包含 merge 迁移，确保拉取到最新代码后再次执行：
