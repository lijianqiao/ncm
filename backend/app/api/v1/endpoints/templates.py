...
路由顺序规则（FastAPI 按定义顺序匹配路由）：
=====================================
1. 根路由: `/`
2. 静态路由: `/batch`, `/recycle-bin`, `/batch/restore`, `/batch/hard`,
            `/export`, `/param-types`, `/examples`, `/extract-vars`, `/v2`
3. 动态路由: `/{template_id:uuid}` 及其子路由、`/v2/{template_id:uuid}`

注意：静态路由必须在动态路由之前定义，否则动态路由会先匹配，导致静态路径被误认为是 UUID 参数。
"""
