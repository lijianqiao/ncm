...
def ping(self) -> dict:
    """
    简单的 Ping 任务，用于验�?Celery 连接�?

    Returns:
        dict: 包含状态和消息的字典�?
    """
    logger.info("Ping 任务被调�?, task_id=self.request.id)
...
def long_running(self, duration: int = 10) -> dict:
    ...
    Returns:
        dict: 包含执行信息的字�?
    """
    logger.info("长时间任务开�?, task_id=self.request.id, duration=duration)
    ...
    logger.info("长时间任务完�?, task_id=self.request.id)
